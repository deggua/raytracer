...
#    define Vector_ExtendBy(T) CONCAT2(Vector_ExtendBy, T)
...
// TODO: improve API
static inline bool Vector_ExtendBy(Tn)(Vector(Tn) * vec, size_t count)
{
    if (Vector_Reserve(Tn)(vec, vec->length + count)) {
        vec->length += count;
        return true;
    } else {
        return false;
