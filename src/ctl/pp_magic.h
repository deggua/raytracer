...
/**
 * Concat any number of symbols with _ separators
 */
#define CONCAT2_(a, b) a##_##b
#define CONCAT2(a, b)  CONCAT2_(a, b)

#define CONCAT_1(a)                             a
#define CONCAT_2(a, b)                          CONCAT2(a, b)
#define CONCAT_3(a, b, c)                       CONCAT2(CONCAT_2(a, b), c)
#define CONCAT_4(a, b, c, d)                    CONCAT2(CONCAT_3(a, b, c), d)
#define CONCAT_5(a, b, c, d, e)                 CONCAT2(CONCAT_4(a, b, c, d), e)
#define CONCAT_6(a, b, c, d, e, f)              CONCAT2(CONCAT_5(a, b, c, d, e), f)
#define CONCAT_7(a, b, c, d, e, f, g)           CONCAT2(CONCAT_6(a, b, c, d, e, f), g)
#define CONCAT_8(a, b, c, d, e, f, g, h)        CONCAT2(CONCAT_7(a, b, c, d, e, f, g), h)
#define COCNAT_9(a, b, c, d, e, f, g, h, i)     CONCAT2(CONCAT_8(a, b, c, d, e, f, g, h), i)
#define CONCAT_10(a, b, c, d, e, f, g, h, i, j) CONCAT2(CONCAT_9(a, b, c, d, e, f, g, h, i), j)

#define OVERLOAD12(a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12, ...) a12
...
/**
 * Given a parenthesized list, get the first element in the list
 * GET((a, b, c)) = a
 */
#define GET_(a, ...) a
...
/**
 * Given a parenthesized list, remove the first element and return a new list of the remaining elements
 * NEXT((a, b, c)) = (b, c)
 */
#define NEXT_(a, ...) (__VA_ARGS__)
#define NEXT(...)     NEXT_ __VA_ARGS__
 ...
 */
#define MAP_TO(...) IF(HAS_ARGS(__VA_ARGS__))(EVAL(MAP_INNER(__VA_ARGS__)))
#define MAP_INNER(op, sep, cur_val, ...) \
    op(cur_val) IF(HAS_ARGS(__VA_ARGS__))(sep() DEFER2(_MAP_INNER)()(op, sep, ##__VA_ARGS__))
 ...
 */
#define MAP_WITH_ID(op, sep, ...) IF(HAS_ARGS(__VA_ARGS__))(EVAL(MAP_WITH_ID_INNER(op, sep, I, ##__VA_ARGS__)))
#define MAP_WITH_ID_INNER(op, sep, id, cur_val, ...) \
    op(cur_val, id) IF(HAS_ARGS(__VA_ARGS__))(sep() DEFER2(_MAP_WITH_ID_INNER)()(op, sep, CAT(id, I), ##__VA_ARGS__))
#define _MAP_WITH_ID_INNER() MAP_WITH_ID_INNER

/**
 * Similar to above but you provide a list of IDs that will be used in sequential order
 * Example:
 *
 *  #define MAKE_VAR(type, name) type name;
 *  MAP_WITH_ID(MAKE_VAR, EMPTY, (first, second, third, fourth), int, bool, char, float)
 *
 * Which expands to:
 *
 *  int first; bool second; char third; float fourth;
 */
#define MAP_WITH_ID_LIST(op, sep, id_list, ...) \
    IF(HAS_ARGS(__VA_ARGS__))(EVAL(MAP_WITH_ID_LIST_INNER(op, sep, id_list, ##__VA_ARGS__)))
#define MAP_WITH_ID_LIST_INNER(op, sep, id, cur_val, ...) \
    op(cur_val, GET(id))                                  \
        IF(HAS_ARGS(__VA_ARGS__))(sep() DEFER2(_MAP_WITH_ID_LIST_INNER)()(op, sep, NEXT(id), ##__VA_ARGS__))
#define _MAP_WITH_ID_LIST_INNER() MAP_WITH_ID_LIST_INNER
 ...
 * The mechanism is analogous to the MAP macro.
 */
#define MAP_PAIRS(op, sep, ...) IF(HAS_ARGS(__VA_ARGS__))(EVAL(MAP_PAIRS_INNER(op, sep, __VA_ARGS__)))
#define MAP_PAIRS_INNER(op, sep, cur_val_1, cur_val_2, ...) \
    op(cur_val_1, cur_val_2) IF(HAS_ARGS(__VA_ARGS__))(sep() DEFER2(_MAP_PAIRS_INNER)()(op, sep, __VA_ARGS__))
 ...
 * The mechanism is analogous to the MAP macro.
 */
#define MAP_SLIDE(op, last_op, sep, ...) IF(HAS_ARGS(__VA_ARGS__))(EVAL(MAP_SLIDE_INNER(op, last_op, sep, __VA_ARGS__)))
#define MAP_SLIDE_INNER(op, last_op, sep, cur_val, ...)                                \
    ...
    (op(cur_val, FIRST(__VA_ARGS__))) IF(NOT(HAS_ARGS(__VA_ARGS__)))(last_op(cur_val)) \
        IF(HAS_ARGS(__VA_ARGS__))(sep() DEFER2(_MAP_SLIDE_INNER)()(op, last_op, sep, __VA_ARGS__))
#define _MAP_SLIDE_INNER() MAP_SLIDE_INNER

/**
 * Strip any excess commas from a set of arguments.
 */
#define REMOVE_TRAILING_COMMAS(...) MAP_TO(PASS, COMMA, __VA_ARGS__)
